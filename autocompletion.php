<?php
    ...
    if (!empty($_POST['recherche'])) {
        ...
        $recherche = $bdd->prepare('SELECT * FROM pilote_moto_gp WHERE nom LIKE "%'.$data.'%" '); //or prenom LIKE "%'.$data.'%" or moto LIKE "%'.$data.'%" or ecurie LIKE "%'.$data.'%" ORDER BY id desc'
        $recherche->execute();
        $recherchePilote = $recherche->fetchAll(PDO::FETCH_ASSOC);
        //var_dump($recherchePilote);
